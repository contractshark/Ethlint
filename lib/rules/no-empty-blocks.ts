...
module.exports = {
  ...
  create(context) {
    ...
    function inspectBlockStatement(emitted) {
      ...
      if (
        emitted.exit ||
        isFallbackFunc(parent) ||
        isPayableFuncOrCons(parent)
      ) {
        return;
      }
    ...
    function inspectConstructorDeclaration(emitted) {
      ...
      if (!emitted.exit) {
        ...
        enclosingContractsOfConstructors[
          node.start
        ] = context.getSourceCode().getParent(node);
        return;
      }
    ...
    // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    let constructorsToCheckForBaseCall = [],
      enclosingContractsOfConstructors = {};
    const similarNodeTypes = [
      'ContractStatement',
      'LibraryStatement',
      'InterfaceStatement',
    ];
