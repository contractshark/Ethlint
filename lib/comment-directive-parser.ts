...
class CommentDirectiveParser {
  ...
  _removeRulesFromLineConfig(line, rules) {
    ...
    this.lineConfigurations[line] = this.lineConfigurations[line].filter(
      (lc) => {
        return !rules.includes(lc);
      },
    );
  }
  ...
  _constructLineConfigurationFromComment(token) {
    ...
    if (text.includes(SDPL)) {
      ...
      if (targetLine < 1) {
        throw new Error(
          `Comment directive "${SDPL}" refers to an invalid line number.`,
        );
      }

      return this._addRulesToLineConfig(
        targetLine,
        this._parseRuleNames(text, SDPL),
      );
    }
    ...
    if (text.includes(SE)) {
      const currLine = astUtils.getEndingLine(token),
        rulesToEnable = this._parseRuleNames(text, SE);
      const objContext = this;
      ...
      this._toEndOfFile(
        currLine + 1,
        (lineNum) => {
          objContext._removeRulesFromLineConfig(lineNum, rulesToEnable);
        },
        // @ts-expect-error ts-migrate(7006) FIXME: Parameter 'lineNum' implicitly has an 'any' type.
      );
    }
